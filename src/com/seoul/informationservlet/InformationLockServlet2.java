...
@WebServlet(value = "/contacts/lock23.action")
public class InformationLockServlet2 extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			resp.sendRedirect("lock.action3?boardno="+boardNo+"&pageno="+pageNo);
			return;
		}
		...
		if (checkPasswd.equalsIgnoreCase(password)) {
			...
			//5. detail.jsp로 이동
			resp.sendRedirect("informationdetail3.action?boardno="+boardNo+"&pageno="+pageNo);
;
			} else {
