...
<script type="text/javascript">
	...
	$(function() {
		$('a#delete').on('click', function(event) {
			...
			if (ok) {
				location.href = "delete35.action?boardno=" + boardNo;
			}
																		...
																		<div id="inputmain">
																			...
																			<div class="buttons">
																				...
																				<%-- <% if ( member.getMemberId().equals( information.getM_MemberId())) { %> --%>
																					...
																					<% if (member == null)  { %>
																					[<a href='list35.action?pageno=<%=request.getAttribute("pageno")%>'>목록보기</a>]
																						<% } else if (member.getMemberId().equals("admin")) { %>
																						[<a id="delete" href='#'	data-boardno="<%=information.getIn_No()%>">삭제</a>]
																						[<a href='update35.action?boardno=<%=information.getIn_No()%>&pageno=<%=request.getAttribute("pageno")%>'>수정</a>]
																						[<a href='list35.action?pageno=<%=request.getAttribute("pageno")%>'>목록보기</a>]
																							<%} else { %>
																							[<a href='list35.action?pageno=<%=request.getAttribute("pageno")%>'>목록보기</a>]
																							<% }	%>
