  ...
  opacity: 0.7;
	...
	$(function() { //jQuery's main function
		$('#write').on('click', function(event) {
			...
			location.href = 'turewrite.action';
		});
...
function openCity(evt, cityName) {
	...
	<!-- -------------------------------------------------------------------------------- -->
		...
		<br><br><br>
		<div type="button" value="글쓰기" >
			
			<input id="write" class="btn" type="button" value="write now" style="font-size:30px;width: 200px; height: 60px; border-radius:4px; border: 1px solid #bbbbbb; background: linear-gradient(#f1f1f1 0%, #ffffff 2px,  #eeeeee 100%); outline:none; " />	
		
		</div>

				
			<br /><br />
			<% List<TureBoard> boards = (List<TureBoard>)request.getAttribute("boards"); %>	
			...
			<% for (int row=0; row < boards.size() / 3 + 1; row++) { %>
			<tr style="padding:15px 15px 15px 15px">
			<% for (int kan=0; kan < 3 && row*3 + kan < boards.size(); kan++) { %>
			<% TureBoard board = boards.get(row*3 + kan); %>
			<td class="procard" style="padding:7px 7px 7px 7px; width:300px;border-radius: 1em ;">
			<div style="width:300px; height:500px; background-color:azure; float:left;"><!-- <input type="hidden"><%= board.getBoardNo() %>  -->
			...
			<!--  <a id="d<%= board.getBoardNo()%>" href="#" data-boardno='<%= board.getBoardNo()%>' data-pageno='<%= request.getAttribute("pageno") %>' style = 'text-decoration:none; vertical-align:middle;font-size:15pt'>
			<%= board.getTitle() %><!-- boarno가지고 detail.action으로 이동 --><!-- request = sevlet에서 넘겨주는 변수 읽을때 -->
		<!--  	</a> -->
		...
		<br><br><br><br><br>
		<div style="text-align:center"><%= request.getAttribute("pager").toString() %></div>
		</div>
