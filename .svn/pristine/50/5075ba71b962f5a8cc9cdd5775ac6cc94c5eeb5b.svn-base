...
@WebServlet(value = "/contacts/eventsearch.action")
public class EventSearchServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		int pagerSize = 3;
		int boardCount = 0;
		String linkUrl = "eventsearch.action";
		...
		List<ContactsBoard> boards = boardService.findBoardList6(search1, search2, first, last);//추가**
		
		boardCount = boardService.getBoardCount6(search1, search2);
		...
		RequestDispatcher dispatcher = 
			req.getRequestDispatcher("/WEB-INF/views/contacts/eventsearch.jsp");
