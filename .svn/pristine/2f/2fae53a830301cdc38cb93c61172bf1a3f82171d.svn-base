...
@WebServlet(value = "/contacts/aftersearch.action")
public class AfterSearchServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		int pagerSize = 3;
		int boardCount = 0;
		String linkUrl = "aftersearch.action";
		...
		List<ContactsBoard> boards = boardService.findBoardList8(search1, search2, first, last);//추가**
		
		boardCount = boardService.getBoardCount8(search1, search2);
		...
		RequestDispatcher dispatcher = 
			req.getRequestDispatcher("/WEB-INF/views/contacts/aftersearch.jsp");
