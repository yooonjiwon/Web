...
@WebServlet(value = "/contacts/contactslist.action")
public class ContactsListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		int pagerSize = 3;
		int boardCount = 0;
		String linkUrl = "contactslist.action";
		...
		List<ContactsBoard> boards = boardService.findBoardList1(first, last);//추가**
		
		boardCount = boardService.getBoardCount1();
		...
		RequestDispatcher dispatcher = 
			req.getRequestDispatcher("/WEB-INF/views/contacts/contactslist.jsp");
