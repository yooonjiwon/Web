...
<head>
	...
	$(function() { // jQuery's main function // html에 하드코딩하는 방식이 좋지 않기때문에 javascript를 분리해서 수행
		$('input#write').on('click', function(event) {
			location.href= 'contactswrite.action';
		});
	});
	...
	$(function() {
		$('button#search').on('click', function(event){
			...
			location.href= 'contactssearch.action?pageno=1&search1=' + search1 + '&search2=' + search2;
		});
                                        ...
                                        <div class="wrapper">
                                            ...
                                            <article class="grid_8 omega">
												<div style="padding-top:25px;text-align:center">
													<!-- location.href= -->
													<%-- <% List<ContactsBoard> boards = (List<ContactsBoard>)request.getAttribute("boards"); --%>
													<table border="1" align="center">
														...
														<tr style="height:25px">
															<td style='text-align:center'><%-- <%= board.getBoardNo() --%>${ board.boardNo }
															...
															<!--  게시글 목록중 하나를 눌렀을 때 내용으로 넘어가는 작업 1단계 Rewrite -->
																<a href='contactsdetail.action?boardno=${ board.boardNo }&pageno=${ pageno }' style='text-decoration: none'>
																	${ board.title }
																</a>
															</c:when>
															<c:when test="${ loginuser.memberId eq 'admin' }">
																<a href='eventdetail.action?boardno=${ board.boardNo }&pageno=${ pageno }' style='text-decoration: none'>
																	${ board.title }
													...
													<div>
														<%-- <% ContactsBoard board = (ContactsBoard)request.getAttribute("board"); --%>
